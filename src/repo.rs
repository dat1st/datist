...
use crate::types::{CookieStore, DatistRepoError, KeychainEntry, KeychainListEntry, Thread, ThreadActionInteractionEntry, ThreadActionTypingEntry, ThreadEntry, ThreadRepo, ThreadXhrStreamEntry, User};
...
pub trait DatistRepo {
    ...
    async fn thread_action_interaction_add_entry(&self, user_id: Uuid, thread_id: Uuid, thread_entry: ThreadActionInteractionEntry) -> Result<(), DatistRepoError>;
    async fn thread_action_interaction_list_entries(&self, user_id: Uuid, thread_id: Uuid, page: u32) -> Result<Vec<ThreadActionInteractionEntry>, DatistRepoError>;
}
...
impl DatistRepo for ThreadRepo {
    ...
    async fn thread_action_interaction_add_entry(&self, user_id: Uuid, thread_id: Uuid, thread_entry: ThreadActionInteractionEntry) -> Result<(), DatistRepoError> {
        ...
        let key = format!("user:{user_id}:threads:{thread_id}:ct_stream:{timestamp}");

        let thread_entry = ThreadActionInteractionEntry {
            created_utc: Some(timestamp),
            ..thread_entry
        };
    ...
    async fn thread_action_interaction_list_entries(&self, user_id: Uuid, thread_id: Uuid, page: u32) -> Result<Vec<ThreadActionInteractionEntry>, DatistRepoError> {
        ...
        let from = format!("user:{user_id}:threads:{thread_id}:ct_stream:");
        let to = format!("user:{user_id}:threads:{thread_id}:ct_stream:{}", u64::MAX);
        ...
        for (key, mut value) in iter {
            ...
            if i >= page * 50 && i < (page + 1) * 50 {
                let entry: ThreadActionInteractionEntry = simd_json::from_slice(value.as_mut())
                    .map_err(|_| DatistRepoError::SerializationError)?;
